...
class PlacementScreen(tk.Frame):  # Screen 2: both players place ships before battle
    ...
    def _place_ai_ships(self):
        """Randomly place ships for player 2 when EASY AI is active."""
        import random
        s = self.app.state
        board = s.p2_board
        ships_list = s.p2_ships
        # iterate lengths 1..num_ships and place each randomly
        for length in range(1, s.num_ships + 1):
            placed = False
            while not placed:
                r = random.randrange(GRID_SIZE)
                c = random.randrange(GRID_SIZE)
                orient = random.choice(["H", "V"])
                if self.can_place(board, r, c, length, orient):
                    coords = self.place_ship(board, r, c, length, orient)
